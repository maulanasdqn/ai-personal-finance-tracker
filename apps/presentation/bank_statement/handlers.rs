...
use crate::infrastructure::{bank_statement::D1BankStatementRepository, workspace::D1WorkspaceRepository};
...
async fn handle_list(req: Request, ctx: RouteContext<()>) -> Result<Response, AppError> {
    ...
    let stmts = D1BankStatementRepository::new(ctx.env.d1("DB").map_err(AppError::from)?).list_by_workspace(wid).await?;
...
async fn handle_get(req: Request, ctx: RouteContext<()>) -> Result<Response, AppError> {
    let wid = ctx.param("workspace_id").ok_or_else(|| AppError::BadRequest("missing workspace_id".into()))?;
    let sid = ctx.param("id").ok_or_else(|| AppError::BadRequest("missing id".into()))?;
    let secret = ctx.env.secret("JWT_SECRET").map_err(AppError::from)?.to_string();
    let user = authenticate(&req, &secret)?;
    D1WorkspaceRepository::new(ctx.env.d1("DB").map_err(AppError::from)?).find_member(wid, &user.user_id).await?.ok_or_else(|| AppError::Forbidden("not a member".into()))?;
    let stmt = D1BankStatementRepository::new(ctx.env.d1("DB").map_err(AppError::from)?).find_by_id(sid).await?.ok_or_else(|| AppError::NotFound("statement not found".into()))?;
    Response::from_json(&BankStatementResponse::from(stmt)).map_err(AppError::from)
}
