...
pub async fn execute(
    ...
    insight_repo: &impl AiInsightRepository,
    api_key: &str,
    ...
    if transactions.is_empty() {
        return Err(AppError::BadRequest(
            "no transactions found in the specified period".into(),
        ));
    }
    ...
    let parsed: serde_json::Value =
        serde_json::from_str(&ai_response).map_err(|_| AppError::Internal)?;
...
fn summarize_transactions(
    transactions: &[crate::domain::transaction::entity::Transaction],
) -> String {
    let total_income: f64 = transactions
        .iter()
        .filter(|t| {
            t.transaction_type == crate::domain::transaction::entity::TransactionType::Income
        })
        .map(|t| t.amount)
        .sum();
    let total_expense: f64 = transactions
        .iter()
        .filter(|t| {
            t.transaction_type == crate::domain::transaction::entity::TransactionType::Expense
        })
        .map(|t| t.amount)
        .sum();
