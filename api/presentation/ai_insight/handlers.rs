...
use crate::infrastructure::{
    ai_insight::D1AiInsightRepository, transaction::D1TransactionRepository,
    workspace::D1WorkspaceRepository,
};
...
async fn handle_generate(mut req: Request, ctx: RouteContext<()>) -> Result<Response, AppError> {
    ...
    if insight_repo
        .was_recently_generated(&wid, AI_COOLDOWN_SECS)
        .await?
    {
        return Err(AppError::TooManyRequests(
            "insights were already generated recently — please wait 5 minutes".into(),
        ));
    }
