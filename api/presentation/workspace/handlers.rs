use crate::application::workspace::use_cases::{
    create,
...
use crate::presentation::workspace::dto::{
    CreateWorkspaceRequest, InviteMemberRequest, MemberResponse, UpdateWorkspaceRequest,
    WorkspaceResponse,
};
...
async fn handle_get(req: Request, ctx: RouteContext<()>) -> Result<Response, AppError> {
    ...
    let repo = workspace_repo!(ctx);
    repo.find_member(id, &user.user_id)
        .await?
    ...
    if let Some(ref name) = body.name {
        if name.is_empty() || name.len() > 80 {
            return Err(AppError::BadRequest(
                "workspace name must be between 1 and 80 characters".into(),
            ));
        }
    }
    let repo = workspace_repo!(ctx);
    let member = repo
        .find_member(id, &user.user_id)
        .await?
        .ok_or_else(|| AppError::Forbidden("not a member".into()))?;
