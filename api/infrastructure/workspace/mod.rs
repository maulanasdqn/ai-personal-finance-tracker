use crate::domain::workspace::entity::{
    MemberRole, NewWorkspace, Workspace, WorkspaceMember, WorkspacePatch,
};
...
impl WorkspaceRepository for D1WorkspaceRepository {
    ...
    async fn list_by_user(&self, user_id: &str) -> Result<Vec<Workspace>, AppError> {
        let cols = WorkspaceRow::COLUMNS
            .iter()
            .map(|c| format!("w.{c}"))
            .collect::<Vec<_>>()
            .join(", ");
        let sql = format!("SELECT {cols} FROM workspaces w JOIN workspace_members wm ON w.id = wm.workspace_id WHERE wm.user_id = ?1 ORDER BY w.created_at DESC");
        let result = self
            .db
            .prepare(&sql)
            .bind(&[user_id.into()])
    ...
    async fn update(
        ...
        if let Some(name) = patch.name {
            set = set.field("name", name);
        }
        if let Some(desc) = patch.description {
            set = set.nullable_field("description", desc);
        }
        if set.is_empty() {
            return self
                .find_by_id(id)
    ...
    async fn list_members(&self, workspace_id: &str) -> Result<Vec<WorkspaceMember>, AppError> {
        Table::<MemberRow>::new(&self.db)
            .find_all(Query::new().eq("workspace_id", workspace_id))
            .await
            .map_err(|_| AppError::Internal)
            .map(|rows| rows.into_iter().map(Into::into).collect())
    }
