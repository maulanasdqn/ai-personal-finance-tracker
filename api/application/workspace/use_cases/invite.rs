...
pub async fn execute(
    ...
    workspace_repo: &impl WorkspaceRepository,
    user_repo: &impl UserRepository,
) -> Result<WorkspaceMember, AppError> {
    let member = workspace_repo
        .find_member(&input.workspace_id, &input.inviter_id)
        .await?
        .ok_or_else(|| AppError::Forbidden("not a workspace member".into()))?;
    if member.role == MemberRole::Member {
        return Err(AppError::Forbidden(
            "only owners and admins can invite".into(),
        ));
    }
    let user = user_repo
    ...
    workspace_repo
        .add_member(&input.workspace_id, &user.id, MemberRole::Member, &now)
        .await
}
