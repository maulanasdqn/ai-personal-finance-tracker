use crate::domain::transaction::entity::{NewTransaction, Transaction, TransactionCategory, TransactionSource, TransactionType};
use crate::domain::transaction::repository::{TransactionFilter, TransactionRepository};
use crate::error::AppError;
use rust_d1_orm::{D1Model, Order, Query, Table, opt_js};
...
impl D1Model for TransactionRow {
    ...
    const COLUMNS: &'static [&'static str] = &[
        "id", "workspace_id", "amount", "currency", "category",
    ...
    fn values(&self) -> Vec<JsValue> {
        vec![
            ...
            self.amount.into(), self.currency.clone().into(),
            self.category.clone().into(), opt_js(self.description.clone()),
            self.transaction_date.clone().into(), self.transaction_type.clone().into(),
            self.source.clone().into(), self.created_by.clone().into(),
...
impl From<TransactionRow> for Transaction {
    fn from(r: TransactionRow) -> Self {
        Self {
            id: r.id, workspace_id: r.workspace_id, amount: r.amount, currency: r.currency,
            category: TransactionCategory::from_str(&r.category).unwrap_or(TransactionCategory::Other),
...
pub struct D1TransactionRepository { db: D1Database }
...
impl TransactionRepository for D1TransactionRepository {
    async fn create(&self, tx: NewTransaction) -> Result<Transaction, AppError> {
        let row = TransactionRow {
            id: tx.id, workspace_id: tx.workspace_id, amount: tx.amount, currency: tx.currency,
            category: tx.category.to_string(), description: tx.description,
            transaction_date: tx.transaction_date, transaction_type: tx.transaction_type.to_string(),
            source: tx.source.to_string(), created_by: tx.created_by,
